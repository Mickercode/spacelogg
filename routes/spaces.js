...
const { upload, getImageUrl } = require('../utils/cloudinary');
...
router.post('/', requireAuth, upload.array('images', 6), async (req, res) => {
  try {
    ...
    const images = (req.files||[]).map(getImageUrl).filter(Boolean);
    const { lastID } = await db.runAsync(
      `INSERT INTO spaces (name,category,description,address,city,lat,lng,price,price_unit,amenities,hours,images,owner_id,status)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'pending')`,
      [name, category, description||'', address, city||'Ibadan', lat?Number(lat):null, lng?Number(lng):null,
       price||'', price_unit||'', amenities||'[]', hours||'{}', JSON.stringify(images), req.user.id]);
...
// PATCH /api/spaces/:id — edit listing (owner only)
router.patch('/:id', requireAuth, upload.array('images', 6), async (req, res) => {
  try {
    ...
    if (!space) return res.status(404).json({ error: 'Space not found' });
    if (space.owner_id !== req.user.id && req.user.role !== 'admin') return res.status(403).json({ error: 'Not authorized' });
    ...
    if (req.files?.length) {
      const newImages = JSON.parse(space.images||'[]').concat(req.files.map(getImageUrl).filter(Boolean));
      updates.push('images=?'); params.push(JSON.stringify(newImages));
    }
